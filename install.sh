...
# ================================================================
# Usage:
#   curl -sSL https://raw.githubusercontent.com/YOUR_ORG/societyflow-agent/main/install.sh | sudo bash
# ================================================================
...
GITHUB_RAW="https://raw.githubusercontent.com/YOUR_ORG/societyflow-agent/main"
...
while true; do
    ...
    # Strip whitespace, carriage returns, and any invisible characters
    AGENT_KEY=$(echo "$AGENT_KEY" | tr -d '[:space:]\r\n')
    # Also strip any BOM or non-printable chars
    AGENT_KEY=$(printf '%s' "$AGENT_KEY" | tr -cd '[:print:]')
    if [ -z "$AGENT_KEY" ]; then
        warn "No key entered. Please paste your key from the portal."
        continue
    fi
    # Accept key with or without SF-AGT- prefix (in case of copy issues)
    if echo "$AGENT_KEY" | grep -qi 'SF-AGT-'; then
        ok "Agent key accepted"
        break
    elif [ ${#AGENT_KEY} -ge 8 ]; then
        # Key entered but without prefix — accept anyway and prepend if needed
        if echo "$AGENT_KEY" | grep -qv 'SF-'; then
            AGENT_KEY="SF-AGT-$AGENT_KEY"
        fi
        ok "Agent key accepted"
        break
    else
        echo ""
        warn "Key not recognized. Expected format: SF-AGT-XXXXXXXXXX"
        echo "  Get your key from: Portal → CCTV → Agent Setup → Generate Agent Key"
        echo ""
    fi
done
