...
add_device() {
    ...
    if [ -n "$SUGGESTED_IP" ]; then
        ...
        read -p "  Correct IP? Press Enter to confirm or type a different IP: " INPUT_IP
        INPUT_IP=$(echo "$INPUT_IP" | tr -d '[:space:]')
        DEVICE_IP="${INPUT_IP:-$SUGGESTED_IP}"
    else
        read -p "  IP address (e.g. 192.168.1.64): " INPUT_IP
    ...
    # Password
    read -s -p "  Password (hidden): " DEVICE_PASS
    echo ""
    [ -z "$DEVICE_PASS" ] && warn "Empty password" || ok "Password: set"

    # RTSP Port
    read -p "  RTSP port? [554]: " INPUT_RTSP
...
for ip in "${DISCOVERED_IPS[@]}"; do
    echo ""
    read -p "  Configure device at $ip? (y/n): " DO_CFG
...
if [ -n "$SUBNET" ] && command -v nmap &>/dev/null; then
    ...
    if [ -n "$ATT_SCAN" ]; then
        ...
        while IFS= read -r ip; do
            [ -z "$ip" ] && continue
            echo "    → $ip (port 4370)"
            read -p "  Configure this device? (y/n): " DO_ATT
