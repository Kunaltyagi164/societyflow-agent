...
# Reconnect stdin to terminal immediately — required when run via curl | bash
# Without this, all 'read' prompts get empty string from the pipe
if [ -t 0 ]; then
    : # already a terminal, nothing to do
else
    exec 0</dev/tty || {
        echo "ERROR: Cannot connect to terminal. Run as two steps:"
        echo "  curl -sSL https://raw.githubusercontent.com/Kunaltyagi164/societyflow-agent/main/install.sh -o install.sh"
...
add_device() {
    ...
    # Password
    read -s -p "  Password (hidden): " DEVICE_PASS </dev/tty
    echo ""
...
add_att_device() {
    ...
    read -s -p "  Device password [blank]: " ATT_PASS </dev/tty
    echo ""
